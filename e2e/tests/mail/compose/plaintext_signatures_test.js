...
async function selectAndAssertSignature (I, mail, name, compare) {
  ...
  I.click(name)
  const result = await grabValueFrom(I, '.io-ox-mail-compose textarea.plain-text')
...
Scenario('Reply to mail with plaintext signature above correctly placed and changed', async ({ I, users, mail }) => {
  const [user] = users
  for (const signature of signatures) {
    const response = await I.haveSnippet(signature)
    signature.id = response.data
...
Scenario('Add and replace signatures with special characters', async ({ I, mail }) => { // at leat one that had to be escaped in a regex
  // at leat one that had to be escaped in a regex
  const first = 'Very original? ...or clever signature?'
  const second = 'Super original and fabulous signature'

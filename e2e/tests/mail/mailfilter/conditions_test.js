...
Scenario('[C7792] Filter mail on sender', async ({ I, users, mail, mailfilter, settings }) => {
  const [user] = users
  await I.haveSetting({
...
Scenario('[C7793] Filter mail on any recipient', async ({ I, users, mail, mailfilter, settings }) => {
  const [user] = users
  await I.haveSetting({
    'io.ox/mail': { messageFormat: 'text' }
  })
...
Scenario('[C7794] Filter mail on to-field', async ({ I, users, mail, mailfilter, settings }) => {
  const [user] = users
  await I.haveSetting({
    'io.ox/mail': { messageFormat: 'text' }
  })
...
Scenario('[C7795] Filter mail on subject', async ({ I, users, mail, mailfilter, settings }) => {
  const [user] = users
  await I.haveSetting({
    'io.ox/mail': { messageFormat: 'text' }
  })
...
Scenario('[C7796] Filter mail on cc-field', async ({ I, users, mail, mailfilter, settings }) => {
  const [user] = users
  await I.haveSetting({
    'io.ox/mail': { messageFormat: 'text' }
  })
...
Scenario('[C7797] Filter mail on header', async ({ I, users, mail, mailfilter, settings }) => {
  const [user] = users
  await I.haveSetting({
    'io.ox/mail': { messageFormat: 'text' }
  })
...
Scenario('[C7800] Filter mail on envelope', async ({ I, users, mail, mailfilter, settings }) => {
  const [user] = users
  await I.haveSetting({
    'io.ox/mail': { messageFormat: 'text' }
  })

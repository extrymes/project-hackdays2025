...
class SenderTrustAnalyzer:
    ...
    def website_exists(self, domain: str) -> bool:
        """Check if the domain has a working website."""
        if not domain:
            return False
        ...
        for protocol in ['http', 'https']:
            try:
                ...
                response = requests.head(url, timeout=3)  # Use HEAD request for efficiency
                if response.status_code < 400:  # 2xx or 3xx status codes
                    return True
            except requests.RequestException:
                continue
                
        return False  # Both HTTP and HTTPS failed
    ...
    def analyze(self, email: str) -> dict:
        ...
        # Check if website exists for non-trusted domains
        website_missing = False
        if not is_trusted and not is_free_provider:
            website_missing = not self.website_exists(domain)
        ...
        if flags["trusted_domain"]:
            ...
            if flags["free_provider"]:
                risk += 5 # Free email providers are slightly suspicious
            
            # Add risk for missing website
            if flags["website_missing"]:
                risk += 10  # Significant risk if domain has no website

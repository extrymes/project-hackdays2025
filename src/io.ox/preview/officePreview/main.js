...
import ox from '@/ox'
import KeyListener from '@/io.ox/core/tk/keys'
import * as util from '@/io.ox/core/util'
import '@/io.ox/preview/officePreview/style.scss'
...
function turnFixedPositioningIntoAbsolutePositioning ($node) {
  $node.find('*').each(function (index, $childNode) {
    ...
    const position = $childNode.css('position')
...
function createInstance (file) {
  ...
  const $pageIndicator = $('<span>').addClass('io-ox-office-preview-page-indicator').text('1')

  const $nextButton = $('<button type="button">').addClass('btn btn-primary disabled')
    .append(createIcon('bi/chevron-right.svg').addClass('icon-white'))
    .on('click', function (e) {
      e.preventDefault()
      app.nextPage()
    })

  const $previousButton = $('<button type="button">').addClass('btn btn-primary disabled')
    .append(createIcon('bi/chevron-left.svg').addClass('icon-white'))
    .on('click', function (e) {
      e.preventDefault()
      app.previousPage()
    })
  ...
  app.showPage = function (pageNumber) {
    ...
    const num = pageNumber + BATCH_SIZE - (pageNumber % BATCH_SIZE)
    fetchPages(num).done(function (doc) {
      const $shownContent = doc[pageNumber].clone()

      // TODO: remove once backend helps here
      // tmp. fix images
      $shownContent.find('img').each(function () {
        const img = $(this); const src = String(img.attr('src'))
        img.attr('src', util.replacePrefix(src, ox.apiRoot))
      })

      container.empty().append($shownContent)

      const centerOffset = ($(window).width() / 2) - ($shownContent.width() / 2)

...
export const getDefaultRulename = function (data) {
  const def = new $.Deferred()
  $.when(resolveDefaultRulename(data)).then(function success (rulename) {
...
export const resolveDefaultRulename = function (data) {
  const test = getFirstTest(data)
  const action = _(data.actioncmds).first()

  if (!test || test.id === true || !action) return gt('New rule')

  const testname = getTestName(test, data.config)
  let actionname = action.id
    ...
    case 'address':
      ...
      if (actionname === 'move') {
        return folderAPI.get(action.into).then(function (data) {
          const arrayOfParts = action.into.split('/'); let title
          arrayOfParts.shift()
...
export const getFirstTest = function (data) {
...
export const getTestName = function (test, config) {
  return _(defaults.getTests(config)).findKey(function (t) {

...
const parse = function (vcard) {
  ...
  for (n in lines) {
    const line = lines[n]
    const isValid = regexps.valid.test(line)
    const isImage = regexps.photo.test(line)
    const isLastLine = regexps.next.test(line)
    let fields = {}
    if (isImage) {
 ...
 * @return {string}         hcard
 */
const hCardContact = function (contact) {
  // http://microformats.org/wiki/hcard
  let output = '<div class="vcard" style="clear:both">'
  let type; let i; let value; let key; let ordered

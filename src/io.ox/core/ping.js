...
const enabled = settings.get('ping/enabled', false)
const interval = settings.get('ping/interval', 30)
let mode = 'none'
let intervalHandle = null

function ping () {
  // don't ping if offline, invalid session, or within first 10 seconds
  if (!ox.session || ox.session === 'unset' || !ox.online || (_.now() - ox.t0) < 10000) return
  http.ping()
}

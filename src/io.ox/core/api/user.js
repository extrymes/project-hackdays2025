...
addReadyListener('rampup', function (rampup) {
  if (rampup.user.id !== ox.user_id) return
  // OXUIB-2127: `internal_userid` is needed when editing groups
  const user = { ...rampup.user, internal_userid: rampup.user.id }
  api.caches.get.add(user)
  api.caches.list.add(user)
})
 ...
 * @return {jQuery.Deferred}   done returns object with timestamp, data
 */
api.update = function (o) {
  if (_.isEmpty(o.data)) return Promise.resolve()

  return import('@/io.ox/contacts/api').then(function ({ default: contactsApi }) {
    ...
    return http.PUT({
      ...
      .then(function () {
        ...
        return api.get({ id: o.id }, false).then(function (data) {
          ...
          const contactsFolder = 'con://0/' + data.folder_id
          return $.when(
 ...
 * @return {jQuery.Deferred}    returns name string
 */
 ...
 * get greeting ('Hello ...')
 * @param  {string}          id of a user
 * @return {jQuery.Deferred}    returns greeting string
 */
...
api.getTextNode = function (id, options) {
  const opt = _.extend({ type: 'name', textZoom: true }, options)
  const node = opt.node || document.createTextNode('')
  api.get({ id })
    .done(function (data) {
      let name = ''
...
api.getLink = function (id, text) {
  ...
  return $('<a href="#" data-detail-popup="halo">').append(text).data({ internal_userid: id })
}
 ...
 */
export const getCurrentUser = api.getCurrentUser = function () {
  return import('@/io.ox/core/settings/user').then(function ({ default: api }) {
...
api.on('update:' + _.ecid({ folder_id: 6, id: ox.user_id }), function () {
  ...
  import('@/io.ox/core/api/account').then(async function ({ default: accountAPI }) {
    await accountAPI.reload()
...
api.getNameExtended = function (data, type) {
  ...
  let userData = data[type + '_from']
  const name = api.checkForName(userData)
...
api.getTextNodeExtended = function (data, type) {
  ...
  if (userData) api.getTextNode(userData, { node })
...
api.checkForName = function (userData) {
  ...
  let result = _.isEmpty(userData.contact) ? userData.display_name : util.getFullName(userData.contact)

...
export default function (e) {
  ...
  const username = $('#io-ox-login-username').val()
  const password = $('#io-ox-login-password').val()
  const form = $(this)
  ...
  if ($.trim(username).length === 0 && !util.isAnonymous()) {
    return util.fail({ error: gt('Please enter your credentials.'), code: 'UI-0001' }, 'username')
  }
  if ($.trim(password).length === 0 && !util.isContinue()) {
    return util.fail({ error: gt('Please enter your password.'), code: 'UI-0002' }, 'password')
  }
...
function storeCredentials (form) {
  const location = window.location.pathname.replace(/[^/]+$/, '') + 'busy.html' // blank does not work in chrome

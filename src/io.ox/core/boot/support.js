...
import { isBrowserSupported, isPlatformSupported } from '@/browser'
...
ext.point('io.ox/core/boot/unsupported').extend(
  ...
  //
  {
    ...
    draw (baton) {
      ...
      if (!_.device('android') || (isPlatformSupported() && !isBrowserSupported())) return
  ...
  //
  {
    ...
    draw (baton) {
      ...
      if (!_.device('ios') || (isPlatformSupported() && !isBrowserSupported())) return
  ...
  //
  {
    ...
    index: 400,
    draw (baton) {
      if (_.device('desktop') || isBrowserSupported()) return
          ...
          .add(
            $('<div data-error="outdated-browser">').text(
              gt('Support starts with Chrome %1$d, Firefox %2$d, Edge %3$d, and Safari %4$d.',
                _.browserSupport.Chrome, _.browserSupport.Firefox, _.browserSupport.Edge, _.browserSupport.Safari
              )
            )
          )
      })
...
export default checkDeviceSupport

...
export default {
  ...
  paste_postprocess (pl, o) {
    ...
    node.find('iframe, object, applet, input, textarea, button, select, canvas, script, noscript, audio, video, img')
      .filter(':not(' +
        [
          'img.emoji',
          `img[src*="${CSS.escape(ox.abs + ox.root + '/api/file')}"]`,
          `img[src*="${CSS.escape(ox.root + '/api/file')}"]`,
          `img[src*="${CSS.escape(ox.abs + ox.root + '/api/image')}"]`,
          `img[src*="${CSS.escape(ox.root + '/api/image')}"]`,
  ...
  htmltotext (string) {
    const node = DOMPurify.sanitize(string, {
    ...
    function processElement (flags, item, i, all) {
      if (item.nodeType === 3) {
        ...
        // remove leading whitespace only after a segment break
    ...
    function processNestedElements (item, flags) {
      const str = traverse(item, { preserveWhitespace: flags.preserveWhitespace || item.nodeName === 'PRE' })

      switch (item.nodeName) {
        case 'DIV': {
          const isCustomBlock = item.classList.contains('io-ox-signature') || item.classList.contains('io-ox-hint')
        ...
        case 'OL': {
          let count = parseInt(item.getAttribute('start') || '1', 10)

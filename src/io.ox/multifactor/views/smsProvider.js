...
const POINT = 'multifactor/views/smsProvider'
let INDEX = 0
...
ext.point(POINT).extend(
  ...
  },
  {
    ...
    id: 'header',
    render (baton) {
      const newCode = baton.model.get('error') && !baton.model.get('error').repeat
      let newDiv = ''
      if (newCode) {
        newDiv = $('<dev class="newSMS">').append(gt('New code sent.') + ' ')
      }
      const label = $('<label for="verification">').append(
        newCode
          ? gt('A new code was sent to your SMS device. Please enter the code.')

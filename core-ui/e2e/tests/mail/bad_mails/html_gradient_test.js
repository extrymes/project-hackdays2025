...
Scenario('[C101616] HTML gradient', async ({ I, users, mail }) => {
  await Promise.all([
    I.haveMail({ path: 'media/mails/C101616.eml' }),
  ...
  within({ frame: '.mail-detail-frame' }, () => {
    I.seeElement(locate('table').withAttr({ class: 'head-wrap', height: '300px' }).as('table'))
  })
})

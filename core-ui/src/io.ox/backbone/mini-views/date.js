...
function createSelect (name, from, to, setter, format) {
  const node = $('<select class="form-control">').attr({ name, title: titles[name], 'aria-label': titles[name] })
  let i = Math.min(from, to)
  const $i = Math.max(from, to)
  ...
  const empty = $('<option>', { value: name === 'year' ? '1604' : '' }).text('')
...
const DateSelectView = AbstractView.extend({
  ...
  getDate () {
    ...
    const date = this.$el.find('.date').val()
  ...
  update () {
    const value = this.model.get(this.name); let d; let year; let text
    ...
    if (_.isNumber(value)) {
      ...
      this.$el.find('.date').val(d.date())
      // disable invalid day fields
      d.date(1)
      d.add(1, 'month')
      d.date(0)
      const validDays = d.date()
      let options = this.$el.find('.date').children().prop('disabled', false)
  ...
  render () {
    const self = this
    const id = this.label.attr('id')
    moment.localeData().longDateFormat('LL').replace(
      /(Y+)|(M+)|(D+)|(?:''|'(?:[^']|'')*'|[^A-Za-z'])+/g,
      function (match, y, m, d) {
        const proto = moment.fn; let node; let year
        if (y) {
          year = moment().year()

...
import { settings } from '@/io.ox/tours/settings'
import openSettings from '@/io.ox/settings/util'

let composeApp
const point = ext.point('io.ox/tours/whats_new')
  ...
  steps (baton) {
    ...
    baton.tour.step()
      ...
      .on('wait', function () {
        ...
        ox.launch(() => import('@/io.ox/mail/main')).then(function () {
...
ext.point('io.ox/tours/whats_new').extend({
  ...
  steps (baton) {
    ...
    // #. Title of tour step, demonstrating options available for 2-step verification
      ...
      .on('wait', () => openSettings('virtual/settings/io.ox/multifactor'))

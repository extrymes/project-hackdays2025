...
import calendarPrint from '@/io.ox/calendar/print'
import api from '@/io.ox/tasks/api'
import '@/io.ox/tasks/util'
...
const states = { 1: gt('Not started'), 2: gt('In progress'), 3: gt('Done'), 4: gt('Waiting'), 5: gt('Deferred') }
const priorities = { 1: gt('Low'), 2: gt('Medium'), 3: gt('High') }
...
function process (data) {
  return calendarPrint.load(data).then(function (unified) {
    return _.extend(unified, {
      ...
      recurrence: RecurrenceRuleMapModel.getRecurrenceString(data),
      state: getState(data),

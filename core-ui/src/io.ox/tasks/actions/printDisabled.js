...
export default {
  multiple (list) {
    if (list.length === 1) {
      ...
      ox.load(() => import('@/io.ox/core/http')).then(function ({ default: http }) {
        const win = print.openURL()
        ...
        http.PUT({
          ...
          const content = $('<div>').append(result)
          const head = $('<div>').append(content.find('style'))
          const body = $('<div>').append(content.find('.print-tasklist'))

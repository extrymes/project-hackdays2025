...
function columnForError (code) {
  const map = { 'OAUTH-0040': 1, 'MSG-0114': 5 }
  return map[code]
}

// some apps have their own handling so the re-authentication dialog is less "In your face!" style
const appsWithOwnHandling = ['mail']
...
ox.on('http:error:OAUTH-0040 http:error:MSG-0114', function (err) {
  ...
  import('@/io.ox/oauth/keychain').then(async function ({ getAPI }) {
    ...
    const account = keychain.accounts.get(err.error_params[columnForError(err.code)])

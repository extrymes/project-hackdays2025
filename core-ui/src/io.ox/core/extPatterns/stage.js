...
const Stage = function (id, options) {
...
// skip other stages
Stage.abort = function (id, runId) {
  if (Stage.pending[id] && Stage.pending[id][runId]) {
    Stage.pending[id][runId].baton.stopPropagation()
  }
}
...
Stage.abortAll = function (id) {
  ...
  return Stage.pending[id][0] ? Stage.pending[id][0].deferred : $.when()
}
...
// promise has resolved successfully. Set's the runId and deferred on the baton.
...
//    beginWith: Pick an extension id in the extensions to start the run with. All extensions
...
Stage.run = function (id, baton, options) {
  ...
  let list = ext.point(id).list()
  const def = $.Deferred()
  // Give this run through a unique ID
  const runId = globalCounter++
  ...
  Stage.pending[id][runId] = baton
  performance.mark(`stages:${id}:start`)
  def.always(function () {
    // Clean up
    delete Stage.pending[id][runId]
    performance.mark(`stages:${id}:end`)
  })

...
const isDeprecatedAPI = !navigator.mediaDevices
let typeCounter = {}
...
export default {
  ...
  getDevices (type) {
    ...
    return enumerateDevices().then(function (devices) {
      ...
      return _.chain(devices)
        .map(function (device) {
          if (type && device.kind.indexOf(type) !== 0) return undefined
          return {
            id: device.deviceId,
  ...
  getStream (constraints) {
    ...
    function onError (e) {
      ...
      def.reject({ type: e.name, message, original_message: e.message })
    }

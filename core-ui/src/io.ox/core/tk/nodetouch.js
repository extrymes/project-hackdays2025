...
function registerTouchHandlers (nodes, _options) {
  ...
  const selector = _options && _options.selector
...
// Unregister all touch handlers at the specified nodes.
 ...
 *
 * @param {string}       phase         The event phase 'cancel', 'start', 'move' or 'end'
 * @param {jQuery.Event} [sourceEvent] The original event object that caused the touch event. Will be used
      ...
      if (fingers === 0) {
        ...
        if (phase === 'start') {
          ...
          if (event.data.horSwipePossible) {
            const horDistance = event.originalEvent.touches[0].pageX - event.data.swipeX
            const absHorDistance = Math.abs(horDistance)
...
/**
 * Disables touch events for all nodes contained in the current collection.
 *
 * @returns {jQuery} A reference to this collection.
 */

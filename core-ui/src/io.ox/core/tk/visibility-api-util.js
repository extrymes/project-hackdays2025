...
let tempHiddenAttribute
let tempVisibilityChangeEvent
...
if (typeof document.hidden !== 'undefined') {
  ...
  tempVisibilityChangeEvent = 'msvisibilitychange' // cSpell:disable-line
...
if (typeof document[tempHiddenAttribute] !== 'undefined') {
  ...
  $(document).on(tempVisibilityChangeEvent, function handleVisibilityChange () {
    const oldState = api.isHidden
    api.isHidden = !!document[api.hiddenAttribute]
    $(api).trigger('visibility-changed', { currentHiddenState: api.isHidden, oldState })
  })
}

...
// helper
const getRight = function (folder, owner, offset) {
  // no folder, no permissions
  if (!folder) return false

  // get bits
  const bits = api.bits(folder, offset)
  // check
  if (bits === 0) {
    // no permissions
    return false
...
const isFolder = function (obj) {
  return 'standard_folder' in obj || obj.folder_id === 'folder'
}

const getFolderId = function (obj) {
...
const getProperties = function (collection) {
  // selection length
  ...
  const folders = _.chain(collection).map(getFolderId).compact().uniq().value()
  ...
  return $.when(api.multiple(folders), userAPI.me()).pipe(function (array, userData) {
    let i = 0; let item = null; let folder = null; const hash = _.toHash(array, 'id'); let folders = false; let items = false; let objectPermission
    ...
    for (; i < $l; i++) {
      ...
      if (isFolder(item)) {
        ...
        const createdBy = item.createdBy && item.createdBy.entity ? item.createdBy.entity : item.created_by

        // bug #52825, check if permission is granted by the folder or by the object
        props.read = props.read && ((objectPermission && objectPermission.bits >= 1) || getRight(folder, createdBy, 7))
        props.modify = props.modify && ((objectPermission && objectPermission.bits >= 2) || getRight(folder, createdBy, 14))
        props.delete = props.delete && ((objectPermission && objectPermission.bits >= 4) || getRight(folder, createdBy, 21))
...
function Collection (list) {
  const items = _.compact([].concat(list))
  const properties = {}
  let resolved = false
...
Collection.Simple = function (items) {
  ...
  const properties = { none: l === 0, some: l > 0, one: l === 1, multiple: l > 1 }
...
function createMatches (properties) {
  return _.memoize(function (str) {
    const condition = String(str || '').replace(/[a-z:]+/ig, function (match) {

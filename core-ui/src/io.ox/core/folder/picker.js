...
export default function (options) {
  ...
  function create () {
    const parentview = tree.getNodeView(tree.selection.get() || tree.root)
    import('@/io.ox/core/folder/actions/add').then(function ({ default: add }) {
      const folderId = mapIds(parentview.folder, true)
      const folder = tree.getNodeView(folderId)
      // if the folder is 'open' and has no subfolders the new created folder appears twice, to prevent this 'close' the folder
      if (folder && folder.options.open && !folder.hasSubFolders()) {
        folder.toggle(false)
      }
      // request and open create-folder-dialog
      add(folderId, { module: o.module === 'calendar' ? 'event' : o.module }).then(
        function (data) {
          // high delay because of tree nodes debounced onSort handler
          _.delay(function () {
            tree.selection.set(data.id)
            const parentNode = tree.getNodeView(data.folder_id)
            const node = tree.getNodeView(data.id)
  ...
  const dialog = new ModalDialog(params)
    .build(function () {
  ...
  let id = o.folder || api.getDefaultFolder(o.module)
  ...
  if (_.device('smartphone')) dialog.$el.find('.modal-dialog').css('width', '95%')
  // get open nodes
  const open = o.settings && _.isString(o.persistent) ? o.settings.get(o.persistent + '/open', []) : []
  ...
  const tree = new TreeView({
    ...
    filter: o.filter,
    flat: !!o.flat,
    // no links like my contact data or subscribe calendar in picker
    noLinks: true,
  ...
  if (o.selection) {
    tree.on('change virtual', function (id) {
      id = mapIds(id)
      const model = api.pool.getModel(id); const data = model.toJSON()
      ...
      let canCreate = data.id === 'default0' && api.altnamespace()
  ...
  return dialog
    ...
    .on('ok', function () {
      const id = tree.selection.get()
      if (id) {
        const mappedId = mapIds(id); const data = api.pool.getModel(id).toJSON()

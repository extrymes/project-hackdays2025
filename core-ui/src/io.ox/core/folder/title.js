...
export default function getFolderTitle (title, max) {
  title = String(title || '').trim()

  // anything to do?
  if (title.length < max) return title

  const leadingDelimiter = /[_-]/.test(title[0]) ? title[0] : false
  const endingDelimiter = /[_-]/.test(title[title.length - 1]) ? title[title.length - 1] : false
  const split = title.split(/[ _-]+/)
  const delimiters = title.split(/[^ _-]+/)
  ...
  while (length > max && split.length > 2) {
    const index = Math.floor(split.length / 2)
  ...
  if (length > max) {
    return _.ellipsis(title, { charpos: 'middle', max, length: Math.floor(max / 2 - 1) })
  }

...
export function perm (bits, offset) {
  return (bits >> offset) & (offset >= 28 ? 1 : 127)
}

export function bits (data, offset) {
  return perm(_.firstOf(data.own_rights, data, 0), offset || 0)
}

export function getDefaultFolder (type) {
  type = type || 'mail'
...
export function registerPool (folderPool) {
  pool = folderPool
}
...
export function is (type, data) {
  let result; let i = 0; let id = ''; let $i; let folders
  ...
  switch (type) {
    ...
    case 'shared-by-me': {
      ...
      if (data.permissions && data.permissions.length >= 1) {
        onlyShowFolder = _(data.permissions).filter(p => p.bits === 1)
      }
    ...
    case 'hidden': {
      const hash = coreSettings.get(['folder/hidden'], {})
    ...
    case 'resourceCalendar': {
      return /^cal:\/\/0\/resource/.test(data.id)
    }
    default:
      return false
  }
...
function canMove (folder, target) {
  ...
  const isExternalRoot = is('external', folder) && folder.virtual_parents[0]
...
export function can (action, data, obj) {
  // check multiple folder?
  if (_.isArray(data)) {
    // for multiple folders, all folders must satisfy the condition
  ...
  // vars
  const rights = data.own_rights
  let bits
  const isSystem = data.standard_folder || is('system', data)
  const isAdmin = perm(rights, 28) === 1
  const isMail = data.module === 'mail'
  // is my folder ?
  const creator = obj ? _.firstOf(obj.created_by, obj.createdBy ? obj.createdBy.entity : undefined, 0) : undefined
  const compareValue = (obj && ox.user_id !== creator) ? 1 : 0
  // switch
  switch (action) {
    case 'read':
...
export function supports (capability, data) {

...
const codes = {
  ...
  find (phonenumber) {
    const part = phonenumber.slice(0, 6)
    if (part[0] !== '+') return
    let match; const hash = _.toHash(codes.hash, 'code')
    // order: more specific -> more general
    // +49123, +4912, +491, +49
    _.each(part, function (char, index) {
      if (match) return
      const scope = part.slice(0, part.length - index)
      match = hash[scope]
    })
    return match
  },

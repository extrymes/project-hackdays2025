...
const label = gt('Image size')

function resize (size, model) {
  const file = model.get('originalFile')
  // no computation necessary without a file
  if (!file) return $.when()
  // no computation if file has size 0, that means it has been deleted in the meantime
  if (file.size === 0) {
    yell('warning', gt('The image cannot not be resized because it was not found on your hard drive.'))
    return $.when()
  }
  // no computation necessary for original, but file upload needs to be triggered
  if (size === 'original') {
  ...
  // add image dimension and check if this image is eligible for resize
  return imageResize.addDimensionsProperty(file).then(function () {
    ...
    const targetDimensions = imageResize.getTargetDimensions(file, size)
...
export default Dropdown.extend({
  ...
  initialize (opt) {
    ...
    const predefined = settings.get('features/imageResize/default', 'original')

...
import config from '@/io.ox/core/config'
...
ext.point('io.ox/core/updates').extend({
  ...
  run () {
    const def = $.Deferred()
    const classicSignatures = config.get('gui.mail.signatures')

    const deferreds = _(classicSignatures).map(function (classicSignature) {

...
const exports = {
  ...
  initialize () {
    ...
    const filter = getFilter(exports.numberFields, '<>', '')
  ...
  findByEmail (str) {
    const email = cleanAddress(str)
    const cached = exports.hash[email]
    if (cached) return $.when(cached)
    const filter = getFilter(exports.emailFields, '=', email)
  ...
  getUserNameNode (userId) {
    const preliminary = String(userId).replace(/\.|(@[^@]+$)/g, ' ').trim()
    const node = document.createTextNode(preliminary)
